...
/**
 * 
 * (Data-)Class for model attributes. Provides means to convert JSON to Object
...
public class ModelAttributes {
	 ...
	 * 
	 * @param jsonModelAttribute
	 *            the attribute as (SyncMeta-compatible) JSON file
	 * 
	 */
	 ...
	 * 
	 * @throws SQLException
	 *             if something went wrong loading the ModelAttributes
	 * 
	 */
	public ModelAttributes(String modelName, Connection connection) throws SQLException {
		...
		PreparedStatement statement = connection.prepareStatement("SELECT * FROM ModelAttributes WHERE modelName=?;");
		...
		statement = connection
				.prepareStatement("SELECT attributeId FROM AttributeToModelAttributes WHERE modelAttributesName = ?;");
	...
	/**
	 * 
	 * Creates a ModelAttributes entry from passed on attributes from a simple
	 * model.
	 * 
	 * @param name
	...
	/**
	 * 
	 * Returns the JSON representation of this model attribute. The
	 * representation is rather specific to SyncMeta and should not be taken as
	 * a generic example of a JSON object representation.
	 * 
	 ...
	 * 
	 * @param connection
	 *            a Connection element passed on from the model class
	 * 
	 * @throws SQLException
	 *             thrown if something goes wrong persisting the model
	 *             attributes
	 * 
	 */
	public void persist(Connection connection) throws SQLException {
		...
		PreparedStatement statement = connection
				.prepareStatement("INSERT INTO ModelAttributes (modelName) VALUES (?);");
		statement.setString(1, this.name);
		// execute query
	...
	public void deleteFromDatabase(Connection connection) throws SQLException {
		PreparedStatement statement = connection.prepareStatement("DELETE FROM ModelAttributes WHERE modelName = ?;");
		statement.setString(1, this.name);
		statement.executeUpdate();
		statement.close();
		for (int i = 0; i < this.attributes.size(); i++) {
