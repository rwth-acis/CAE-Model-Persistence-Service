...
public class ModelPersistenceService extends RESTService {
	...
	private static String semanticCheckService = "";
	private static String codeGenerationService = "";
	...
	public static class Resource {
		...
		public Response postModel(String inputModel) {
			...
			if (!ModelPersistenceService.semanticCheckService.isEmpty()) {
				this.checkModel(model);
			}

			// call code generation service
			if (!ModelPersistenceService.codeGenerationService.isEmpty()) {
				try {
		 ...
		 *            the model as a JSON string
		 * 
		 * @return HttpResponse containing the status code of the request and
		 *         (if successful) the model as a JSON string
		 * 
		 */
		@GET
		 ...
		 * 
		 * @return HttpResponse containing the status code of the request and
		 *         (if the database is not empty) the model-list as a JSON array
		 * 
		 */
		...
		/**
		 * 
		 * Loads a model from the database (by calling the respective resource)
		 * and sends it to the code generation service, requesting a
		 * Communication Model view to be displayed in SyncMeta's application
		 * editor view.
		 * 
		 * TODO: Not tested..
		 * 
		 ...
		 * 
		 * @return HttpResponse containing the status code of the request and
		 *         the communication view model as a JSON string
		 */
		@GET
			...
			for (SimpleEntityAttribute attribute : appModel.getAttributes()) {
				if (attribute.getName().equals("type") && attribute.getValue().equals("application")) {
					...
					try {
						...
						SimpleModel communicationModel = (SimpleModel) Context.getCurrent().invoke(
								ModelPersistenceService.codeGenerationService, "getCommunicationViewOfApplicationModel",
								payload);
		 ...
		 * model) and serves them automatically, such that the rest of this
		 * service does not have to deal with this "special case".
		 * 
		 * @param methodName
		...
		private Model callCodeGenerationService(String methodName, Model model) throws CGSInvocationException {
			...
			try {
				Serializable[] payload = { modelsToSend };
				String answer = (String) Context.getCurrent().invoke(ModelPersistenceService.codeGenerationService,
						methodName, payload);
		...
		/**
		 * 
		 * Performs the semantic check (if specified) on the model, without
		 * storing it
		 * 
		 * @param inputModel
		 *            the model as a JSON string
		...
		/**
		 * 
		 * Returns the API documentation for a specific annotated top level
		 * resource for purposes of the Swagger documentation.
		 * 
		 * Note: If you do not intend to use Swagger for the documentation of
		 * your Service API, this method may be removed.
		 * 
		 * Trouble shooting: Please make sure that the endpoint URL below is
